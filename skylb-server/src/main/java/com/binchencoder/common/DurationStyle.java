...
public enum DurationStyle {
  ...
  SIMPLE("^([\\+\\-]?\\d+)([a-zA-Z]{0,2})$") {
    ...
    public Duration parse(String value, ChronoUnit unit) {
      try {
        ...
        Preconditions.checkState(matcher.matches(), "Does not match simple duration pattern");
        String suffix = matcher.group(2);
        return (null != suffix && !suffix.isEmpty() ? DurationStyle.Unit
            .fromSuffix(suffix) : DurationStyle.Unit
            .fromChronoUnit(unit))
    ...
    public String print(Duration value, ChronoUnit unit) {
      return DurationStyle.Unit.fromChronoUnit(unit).print(value);
    }
   ...
   *
   * @param value the value to parse
   * @param unit the duration unit to use if the value doesn't specify one ({@code null} will
   * default to ms)
   * @return a duration
   */
  ...
  enum Unit {
    ...
    public static DurationStyle.Unit fromChronoUnit(
        ChronoUnit chronoUnit) {
      if (chronoUnit == null) {
        return DurationStyle.Unit.MILLIS;
      }
      for (DurationStyle.Unit candidate : values()) {
    ...
    public static DurationStyle.Unit fromSuffix(String suffix) {
      for (DurationStyle.Unit candidate : values()) {
